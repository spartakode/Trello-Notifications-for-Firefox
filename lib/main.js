...
function constructMessage(jsonCode){
    if(jsonCode.type == "changeCard"){
        ...
        else if(jsonCode.data.old.closed != undefined){
            if(jsonCode.data.old.closed == true){
                return "Archived ";
            }
        }
    ...
    else if(jsonCode.type == "updateCheckItemStateOnCard")
    {
        checkedState = "";
        if(jsonCode.data.checkItem.state == "complete")
        {
        ...
        return (checkedState + jsonCode.data.checkItem.name + " on " + jsonCode.data.card.name + " on " + jsonCode.data.board.name)
    }
